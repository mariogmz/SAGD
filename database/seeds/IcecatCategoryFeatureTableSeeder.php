...
class IcecatCategoryFeatureTableSeeder extends Seeder {
    ...
    private function getCategoriesFeaturesFromSource() {
        ...
        return  $this->icecat_feed->getCategoriesFeatures(true);
    ...
    private function insertIntoDB($categories_features) {
        ...
        $progress_bar->setFormat("<info>Seeding:</info> Icecat Categories Features: [%bar%] %percent:3s%% %elapsed:6s%/%estimated:-6s%");
        $progress_bar->start();
        DB::statement('SET FOREIGN_KEY_CHECKS=0;');
        foreach ($categories_features as $category_feature) {
            App\IcecatCategoryFeature::create($category_feature);
